...
const BlogsTwo = () => {
  ...
  const [tags, setTags] = useState([]); // You can update this if you have a tags API or static data

  useEffect(() => {
    // Fetch blog data from the API
    const fetchBlogs = async () => {
      try {
        const response = await fetch("https://indianmedicalcounsellor.in/wp-json/wp/v2/posts");
        const data = await response.json();
        // Map the API response to the format required
        const mappedBlogs = data.map(blog => {
          const yoastJson = blog.yoast_head_json;
          const image = yoastJson.og_image?.[0]?.url || ""; // Get the image URL from yoast_head_json
          return {
            id: blog.id,
            title: yoastJson.title || blog.title.rendered,
            date: new Date(blog.date).toLocaleDateString(),
            imageSrc: image || "", // Handle case where no image is available
            category: blog.class_list.includes("category-news-and-updates") ? "News" : "Update", // Adjust category based on class_list
          };
        });
