...
const BlogDetails = () => {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        const response = await axios.get(`https://indianmedicalcounsellor.in/wp-json/wp/v2/posts/${id}`);
  ...
  // Extract data from the blog response
  const {
  ...
  const title = yoastJson?.title || blog.title.rendered;
  const imageSrc = yoastJson?.og_image?.[0]?.url || "";
  const author = yoastJson?.author || "Unknown Author";
  const publishedDate = new Date(date).toLocaleDateString();
  const category = blog.class_list.includes("category-news-and-updates") ? "News" : "Update";
            ...
            <div className="row justify-center text-center">
              <div className="col-auto">
                <div>
                  ...
                  <p className="page-header__text">{publishedDate}</p>
                </div>
              </div>
            </div>
          </div>
                ...
                <div className="d-flex border-bottom-light border-top-light py-30">
                  <div className="">
                    <div
                      className="bg-image size-70 rounded-full js-lazy"
                      style={{
                        backgroundImage: `url('https://secure.gravatar.com/avatar/daf3185959fd69176ed065612f51b691?s=96&d=mm&r=g')`,
                      }}
                    ></div>
                  </div>

                  <div className="ml-30 md:ml-20">
                    <h4 className="text-17 lh-15 fw-500">{author}</h4>
                    <div className="mt-5">Medical Assistant</div>
                    <p className="mt-25">
                      Etiam vitae leo et diam pellentesque porta. Sed eleifend
                      ultricies risus, vel rutrum erat commodo ut. Praesent
